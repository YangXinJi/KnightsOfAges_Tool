...
public class RadarView extends View {
    ...
    private int valueColor = Color.valueOf(R.color.colorPrimary).toArgb();//数据区颜色
    ...
    private float textSize = 16f;//字体大小sp
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        radiusStorage = radius = (Math.min(h, w) >> 1) * 0.6f;
        mWidth = w;
        mHeight = h;
        postInvalidate();
    ...
    public void refresh(int index,String s){
        RadarData data=new RadarData(dataList.get(index).getTitle(),Double.parseDouble(s)*100/1.3);
    ...
    private void drawSpiderweb(Canvas canvas) {
        ...
        float r = radiusStorage / (count - 1);//蜘蛛丝之间的间距
    ...
    private void drawText(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            float x = (float) ((radiusStorage + fontHeight * 2) * Math.sin(angle / 2 + angle * i));
            float y = (float) ((radiusStorage + fontHeight * 2) * Math.cos(angle / 2 + angle * i));
